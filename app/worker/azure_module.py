...
def store_dummy_monthly_cost(conn, cloud="Azure"):
    ...
    for month_start in months:
        ...
        """, rows)
        conn.commit()
        log.info(f"[Azure] Stored dummy cost data for {month_str}")
...
def store_dummy_server_status(conn, cloud="Azure"):
    ...
    ⚠️ Replace with Azure Resource Manager API in future.
    """
    retrieved_at = datetime.utcnow()
    cur = conn.cursor()
    ...
    conn.commit()
    cur.close()
    log.info(f"[Azure] Stored dummy server status data")
